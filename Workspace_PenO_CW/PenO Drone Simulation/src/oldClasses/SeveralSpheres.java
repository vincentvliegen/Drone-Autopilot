...
import DroneAutopilot.mission.Mission;
...
public class SeveralSpheres extends Mission {
	...
	private final static float distanceToArrival = 0.55f;// TODO afstellen, straal is 0.5
	private final static int timeToRefresh = 10;


	
	public SeveralSpheres(DroneAutopilot droneAutopilot) {
		super(droneAutopilot);
		this.closestOrbs = new ClosestOrbs(this.getDrone());
		this.worldScan = new WorldScan(this.getDrone());
		setRefreshCounter(0);
		setFirstTime(true);
		setTargetLostCounter(0);
	}
	...
	public void execute() {
		...
		//TODO movetotarget er uit halen
...
//				firstTimeSinceScan();
//			} else if (!this.getMoveToTarget().isScanning() && this.getRefreshCounter() >= getTimeToRefresh()) {// wanneer de drone de 1ste bol in zicht heeft en er naar toe vliegt, herbereken de 1ste en 2de bol op geregelde tijdstippen
//				refreshWhenFlying();
//			}
