...
//			temp[1] = 1-temp[1];
		...
		if((getPolyhedronType() == PolyhedronType.OBSTACLE && temp[2] > 0.55)){
			temp[2] -= 0.55;
			hasChanged = true;
		}
		else if(getPolyhedronType() == PolyhedronType.TARGET && temp[2] <= 0.45) {
			temp[2] += 0.55;
			hasChanged = true;
		}
		
		//enkel als er iets gewijzigd is, ook effectief kleuren aanpassen, anders gewoon overhead
		if(hasChanged) {
