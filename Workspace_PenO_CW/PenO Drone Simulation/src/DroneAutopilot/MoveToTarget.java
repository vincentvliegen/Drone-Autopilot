...
public class MoveToTarget {
	...
	public void execute() {
		ArrayList<int[]> leftCameraList = this.getImageCalculations()
				.getRedPixels(this.getDrone().getLeftCamera());
		ArrayList<int[]> rightCameraList = this.getImageCalculations()
				.getRedPixels(this.getDrone().getRightCamera());
		if (this.checkRoll()) {
	...
	public void targetVisible(ArrayList<int[]> leftCamera,
		...
		if (this.getPhysicsCalculations().horizontalAngleDeviation(cogLeft,
				cogRight) >= underBoundary
			...
			this.flyTowardsTarget(cogLeft, cogRight);
				...
				cogLeft, cogRight) <= underBoundary) {
			this.getDrone().setYawRate(-this.getDrone().getMaxYawRate());
		} else if (this.getPhysicsCalculations().horizontalAngleDeviation(
				cogLeft, cogRight) >= upperBoundary)
			this.getDrone().setYawRate(this.getDrone().getMaxYawRate());
	...
	public void flyTowardsTarget(float[] cogL, float[] cogR) {
		float halfAngleView = this.getDrone().getLeftCamera()
				.getVerticalAngleOfView() / 2;
		// System.out.println("depth " +
		// this.getPhysicsCalculations().getDepth(cogL, cogR));
		...
		} else if (this.getPhysicsCalculations().getVisiblePitch(cogL, cogR)
				- this.getPhysicsCalculations().verticalAngleDeviation(cogL) >= 0) {
			this.getDrone().setPitchRate(this.getDrone().getMaxPitchRate());
			this.getDrone().setThrust(
					Math.min(this.getPhysicsCalculations().getThrust(cogL),
							this.getDrone().getMaxThrust()));
		} else if (this.getPhysicsCalculations().getVisiblePitch(cogL, cogR)
				- this.getPhysicsCalculations().verticalAngleDeviation(cogL) <= underBoundary) {
			this.getDrone().setPitchRate(-this.getDrone().getMaxPitchRate());
			this.getDrone().setThrust(
					Math.min(this.getPhysicsCalculations().getThrust(cogL),
							this.getDrone().getMaxThrust()));
		} else if (this.getPhysicsCalculations().getVisiblePitch(cogL, cogR) == PhysicsCalculations
				.getDecelerationFactor() * halfAngleView) {
			this.getDrone().setPitchRate(-this.getDrone().getMaxPitchRate());
			this.getDrone().setThrust(
					Math.min(this.getPhysicsCalculations().getThrust(cogL),
							this.getDrone().getMaxThrust()));
	...
	public void updateSpeed(float[] cogL, float[] cogR) {
		...
		this.setSpeed(this.getPhysicsCalculations().calculateSpeed(time,
				distance));
	}

	public void updateAcceleration(float[] cog) {
		this.setAcceleration(this.getPhysicsCalculations()
				.calculateAcceleration(cog));
	}
	...
	public void startDeceleration(float[] cogL, float[] cogR) {
		...
		// System.out.println(this.getPhysicsCalculations().getDepth(cogL,
		// cogR));
		...
		if (hover) {
			...
			if (this.getDrone().getPitch() > 0) {
				this.getDrone()
						.setPitchRate(-this.getDrone().getMaxPitchRate());
				this.getDrone().setThrust(
						this.getPhysicsCalculations().getThrust(cogL));
			} else if (this.getDrone().getPitch() <= 0
					&& this.getDrone().getPitch() > -5f) {
				this.getDrone()
						.setPitchRate(-this.getDrone().getMaxPitchRate());
				float thrust = (float) (Math.abs(this.getDrone().getGravity())
						* this.getDrone().getWeight() / Math.cos(Math
						.toRadians(this.getDrone().getPitch())));
